...
// ORDER BY `product_id` ASC
    ...
    $sql = "(SELECT p.product_id,p.image_url, p.product_name, p.quantity, SUM(po.quantity) as quantity_sold,
     p.price FROM product p, product_order po WHERE p.product_id=po.product_id GROUP BY p.product_id)
     UNION (SELECT product_id, image_url, product_name, quantity,NULL, price FROM product WHERE product_id NOT IN
     (SELECT p.product_id FROM product p, product_order po WHERE p.product_id=po.product_id GROUP BY product_id))
     ORDER BY `product_id` ASC";
    $query_result = mysqli_query($conn, $sql);
    while($record = mysqli_fetch_assoc($query_result)) {
      $products[] = $record;
                                        ...
                                        <tbody>
                                            <?php
                                                for($i=0;$i<count($products);$i++){
                                                    ...
                                                    $quantity_sold=$products[$i]['quantity_sold'];
